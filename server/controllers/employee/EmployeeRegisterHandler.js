...
const EmployeeRegisterHandler = (app, db) => {
  app.post('/reg/emp', (req, res) => {
    ...
    const empImage = req.body.empImage;

    //query
    const sqlInsert1 =
      'INSERT INTO emp_details (empName,empMail,empPhone,empAddress,image) VALUES (?,?,?,?,?)';

    const sqlInsert2 = 'INSERT INTO emp_login (emp_id,userName,password) VALUES (?,?,?)';

    const sqlDelete = 'DELETE  FROM emp_details WHERE emp_id= ?';

    //s
    db.query(sqlInsert1, [empName, empMail, empPhone, empAddress, empImage], (err, result) => {
      if (err) {
        console.log(err + "THAT'S AN ERROR!!!");
      } else {
        var emp_id = result.insertId;
        //
        db.query(sqlInsert2, [emp_id, empUserName, empPassword], (err, result1) => {
          if (err) {
            //
            console.log(err);
